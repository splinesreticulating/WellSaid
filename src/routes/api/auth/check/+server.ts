...
export const GET: RequestHandler = async ({ cookies }) => {
    if (!JWT_SECRET) {
        ...
        return new Response(JSON.stringify({ authenticated: false, error: 'Server configuration error' }), {
            status: 500,
    ...
    if (!token) {
        return json({ authenticated: false, timestamp: new Date().toISOString(), reason: 'No token provided' }, { status: 401 });
    }
    ...
    try {
        ...
        return json({ authenticated: false, timestamp: new Date().toISOString(), reason }, { status: 401 });
    }
}
